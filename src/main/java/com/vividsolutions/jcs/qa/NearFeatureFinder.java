...
public class NearFeatureFinder {
     ...
     */
    public List<Feature> findNearFeatures(Feature f, double maxDist) {
        Geometry g = f.getGeometry();
        Envelope envelope = g.getEnvelopeInternal();
        Envelope searchEnvelope = EnvelopeUtil.expand(envelope, maxDist);
        List<Feature> candidates = ifc.query(searchEnvelope);
        List<Feature> nearFeatures = new ArrayList<>();
    ...
    public List findNearByEnvelope(Feature f, FeatureCollection fc, double maxDist) {
        ...
        List<Feature> result = fc.query(searchEnv);
        return result;
    }
